...
| 7 | **Rayleigh–Taylor and Kelvin–Helmholtz** — one dispersion relation with four terms, the capillary and gravitational cutoffs at opposite ends of the spectrum, the non-linear mixing law and the Richardson number; a laser experiment at the National Ignition Facility confirms h = α_B A g t² and refutes its own abstract's claim to have settled α_B, which leaves an astrophysical mixing width uncertain by a factor 2, while Kelvin–Helmholtz vortices on a coronal mass ejection yield no confirmation but do bound the field along the wavevector at 0.0675 gauss for equal fields on both sides, 0.0954 gauss for a one-sided field. The Rayleigh–Taylor half has no astronomical check; the module says so | ready |
