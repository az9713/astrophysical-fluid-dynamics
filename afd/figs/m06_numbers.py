...
def main():
    ...
    P(f'  {plateau:.4f}.')
    band = ((st['rfrac'] >= st['rfrac'][iA]) & (st['rfrac'] <= st['rfrac'][iB])
            & np.isfinite(st['grad']))
    idip = int(np.arange(len(st['grad']))[band][np.argmin(st['grad'][band])])
    P(f'  The interval is not monotone: grad dips to {st["grad"][idip]:.4f} at '
      f'{st["rfrac"][idip]:.4f} R on')
    P(f'  the way, so measured from that minimum the factor is '
      f'{st["grad"][iB]/st["grad"][idip]:.3f}.  Quote whichever')
    P('  the prose uses, and say which.')
    ...
    P('    asymmetry sets the DIRECTION of the two measured departures in')
    P('    PART G.  Do not read them as one quantity:')
    P(f'      Heiles & Troland {100*HT03_F_UNSTABLE:.0f}% is a WARM-medium COLUMN fraction')
    P('        inside an unstable TEMPERATURE band, where the gas may sit')
    P('        exactly on the unstable branch of the curve;')
    P(f'      Jenkins & Tripp {100*JT11_FRAC_BELOW_PMIN:.0f}% is a COLD-medium MASS fraction below a')
    P('        PRESSURE edge, where the curve has no cold branch at all.')
    P('    What the cooling lengths explain is why a warm departure can be')
    P('    sustained while a cold excursion can only be momentary, not a')
    P('    ratio between those two published numbers.')
